...
#if defined(NDEBUG)
# error "Aubiz cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aubiz: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aubiz Signed Message:\n";
...
{
    // Aubiz: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Aubiz: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Aubiz: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aubiz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aubiz: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Aubiz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AubizMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AubizMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AubizMiner(CWallet *pwallet)
{
    printf("AubizMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aubiz-miner");
    ...
    try { loop {
        ...
        printf("Running AubizMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AubizMiner, pwallet));
}
